...
public class PricingController {
    ...
    public ResponseEntity<List<PrecioBase>> obtenerHistorialPorProductoId(@PathVariable Integer productoId) {
        List<PrecioBase> historial = precioBaseService.obtenerHistorialDePreciosPorProducto(productoId);
        return ResponseEntity.ok(historial);
    }

    @PostMapping("/producto/{productoId}")
    public ResponseEntity<PrecioBase> cambiarPrecioProducto(@PathVariable Integer productoId, @RequestBody PrecioBase nuevoPrecio) {
        double monto = nuevoPrecio.getMonto();
        PrecioBase precioActualizado = precioBaseService.actualizarPrecioParaProducto(productoId, monto);
        return new ResponseEntity<>(precioActualizado, HttpStatus.CREATED);
    }
}